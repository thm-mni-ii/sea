...
void Sealib::SimpleTrail::insertSubTrail(const SimpleTrail &subTrail, uint64_t idx) {
    std::vector<std::tuple<uint64_t, uint64_t>> oldTrail = trail;
    ...
    for (uint64_t i = idx + subTrail.getTrail().size(); i < trail.size(); i++) {
        std::tuple<uint64_t, uint64_t> oldVal = oldTrail[i - subTrail.getTrail().size()];
        trail[i] = oldVal;
    }
}

void Sealib::SimpleTrail::addArc(std::tuple<uint64_t, uint64_t> arc) {
    trail.push_back(arc);
}

const std::vector<std::tuple<uint64_t, uint64_t>> &Sealib::SimpleTrail::getTrail() const {
    return trail;
}

Sealib::SimpleTrail::SimpleTrail() = default;

std::tuple<uint64_t, uint64_t> Sealib::SimpleTrail::getOutgoingFrom(uint64_t u) const {
    for (uint64_t i = 0; i < trail.size(); i++) {
...
uint64_t
Sealib::SimpleTrail::getFirstIndexOf(std::tuple<uint64_t, uint64_t> arc) const {
    for (uint64_t i = 0; i < trail.size(); i++) {
        if (trail[i] == arc) {
            return i;
        }
