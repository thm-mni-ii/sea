...
// Compare a standard with a space-efficient BFS run and check that they give
// the same results
TEST(BFSTest, comparison) {
    for (uint64_t a = 0; a < GRAPHCOUNT; a++) {
        ...
        BFS b1(g, [&c1](uint64_t) { c1++; },
               [&c2](uint64_t, uint64_t) { c2++; });
        SimpleBFS b2(g, [&c3](uint64_t) { c3++; },
                     [&c4](uint64_t, uint64_t) { c4++; });
        b1.init();
        b2.init();
        do {
            while (b1.more()) {
...
// Check that the BFS correctly identifies isolated vertices as components
TEST(BFSTest, nextComponent) {
    uint64_t c1 = 0, c2 = 0;
    DirectedGraph g = GraphCreator::kOutdegree(ORDER, 0);
    BFS bfs(g, [&c1](uint64_t) { c1++; }, [&c2](uint64_t, uint64_t) { c2++; });
    uint64_t cc = 0;
...
// Check that the simple BFS correctly identifies isolated vertices as
// components
TEST(SimpleBFSTest, nextComponent) {
    uint64_t c1 = 0, c2 = 0;
    DirectedGraph g = GraphCreator::kOutdegree(ORDER, 0);
    SimpleBFS bfs(g, [&c1](uint64_t) { c1++; },
                  [&c2](uint64_t, uint64_t) { c2++; });
    uint64_t cc = 0;
    bfs.forEach([&cc](std::pair<uint64_t, uint64_t>) { cc++; });
    EXPECT_EQ(cc, ORDER);
