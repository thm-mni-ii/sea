...
Sealib::RecursiveDyckMatchingStructure::RecursiveDyckMatchingStructure(
    const Sealib::Bitset<uint8_t> &word_, uint32_t recursions)
    ...
    if (maxRank > 0) {
        ...
        if (recursions > 0) {
            pioneerMatchingStructure =
                new Sealib::RecursiveDyckMatchingStructure(pioneerWord,
                                                           recursions - 1);
        } else {
            pioneerMatchingStructure =
                new Sealib::NaiveDyckMatchingStructure(pioneerWord);
        }
    }
}
...
Sealib::RecursiveDyckMatchingStructure::initializePioneerRankSelectBitset() {
    ...
    for (uint32_t i = 0; i < segments; i++) {
        ...
        uint64_t leftPioneer =
            LocalDyckTable::getLocalData(segment).leftPioneer;
        uint64_t rightPioneer =
            LocalDyckTable::getLocalData(segment).rightPioneer;

        if (leftPioneer != (uint8_t)-1) {
            if (!pioneerRankSelectBitset[beg + leftPioneer]) {
                pioneerRankSelectBitset[beg + leftPioneer].flip();

                uint64_t matchedPioneer = DyckMatchingStructure::getMatchNaive(
                    word, beg + leftPioneer);
...
uint64_t Sealib::RecursiveDyckMatchingStructure::getMatch(uint64_t idx) {
    ...
    if (word[idx]) {  // opening global
        ...
        int pDepth =
            LocalDyckTable::getLocalData(pSeg).localDepths[pSegmentIdx];
        int bDepth =
            LocalDyckTable::getLocalData(pSeg).localDepths[bSegmentIdx];

        int depthDiff = bDepth - pDepth;

        uint64_t pMatchSegment = static_cast<uint64_t>(pMatch / segmentLength);
        uint64_t matchSegmentIdx =
            static_cast<uint64_t>(pMatch % segmentLength);
        ...
        if (pMatch < segmentLength * segments) {
            ...
            int pMatchDepth = LocalDyckTable::getLocalData(pMatchSeg)
                                  .localDepths[matchSegmentIdx];
            for (int64_t i = static_cast<int64_t>(matchSegmentIdx); i >= 0;
                 i--) {
                int candidateDepth =
                    LocalDyckTable::getLocalData(pMatchSeg).localDepths[i];
                int candidateDepthDifference = pMatchDepth - candidateDepth;
                if (candidateDepthDifference == depthDiff) {  // i is match
                    return (pMatchSegment * segmentLength) +
...
Sealib::RecursiveDyckMatchingStructure::RecursiveDyckMatchingStructure(
    Sealib::Bitset<uint8_t> &&word_, uint32_t recursions)
    : DyckMatchingStructure(word_),
      segments(static_cast<uint32_t>(word_.size() / segmentLength)),
      lastSegment(static_cast<uint8_t>(word_.size() % segmentLength)),
      pioneerRankSelect(initializePioneerRankSelectBitset()),
    ...
    if (maxRank > 0) {
        ...
        if (recursions > 0) {
            ...
            pioneerMatchingStructure =
                new Sealib::DyckMatchingStructure(std::move(pioneerWord));
        }
    }
}
