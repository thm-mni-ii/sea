...
uint ChoiceDictionaryDummy::next(){
	return current_position++;
}

void ChoiceDictionaryDummy::setColor(uint v,uint color){
	insert(v,color);
}

uint ChoiceDictionaryDummy::color(uint v){
	return get(v);
}
